...
export function openFeedbackReport(input: FeedbackReportInput) {
  ...
  <section class="page"><h2>6. Action Plan</h2>${actions.length ? table(['Action','Category','Priority','Status','Progress','Target','Expected outcome'],actions.map(item => [escapeHtml(item.title || item.issue),escapeHtml(item.category || 'General'),escapeHtml(item.priority),escapeHtml(item.status),`${item.completion_percentage}%`,date(item.target_completion_date),paragraph(item.expected_outcome)])) : '<p class="muted">No action plans recorded.</p>'}</section>
  <section class="page"><h2>7. Iterations</h2>${iterations.length ? table(['#','Iteration','Phase','Period','Summary','Improvements','Validation outcome','Status'],iterations.map(item => [String(item.iteration_number ?? ''),escapeHtml(item.iteration_name || item.phase),escapeHtml(item.phase),`${date(item.start_date)} - ${date(item.end_date)}`,paragraph(item.summary),paragraph(item.improvements_made),paragraph(item.validation_result),escapeHtml(item.status)])) : '<p class="muted">No iterations recorded.</p>'}</section>
  <section class="page"><h2>8. Overall Summary</h2><div class="card"><h3>Validation result</h3><p>The report contains ${rows.length} participant response${rows.length === 1 ? '' : 's'}, with an overall satisfaction result of <strong>${rating(overview.satisfaction)}</strong>, task completion of <strong>${percent(overview.taskCompletion)}</strong>, and recommendation intent of <strong>${percent(overview.recommendation)}</strong>.</p><p>These results should be interpreted alongside the direct participant evidence and the documented action plan. The next validation cycle should test the prioritized refinements with an expanded participant sample.</p></div></section>
